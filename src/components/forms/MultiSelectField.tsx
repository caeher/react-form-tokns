...
function renderMultiOption(option: ComboboxOption, disabled: boolean): ReactNode {
  ...
  return (
    <div className="flex min-w-0 items-center gap-3">
      <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5">
        {option.image ? (
          ...
          <span className={disabled ? 'text-slate-300 dark:text-slate-700' : 'text-slate-400 dark:text-slate-500'}>#</span>
        )}
      </div>
...
export const MultiSelectField = forwardRef<HTMLSelectElement, MultiSelectFieldProps>(({
  ...
  const triggerContent = selectedOptions.length === 0
    ...
    : <span className={`truncate ${disabled ? 'text-slate-400 dark:text-slate-600' : 'text-slate-900 dark:text-slate-100'}`}>{triggerLabel}</span>;
