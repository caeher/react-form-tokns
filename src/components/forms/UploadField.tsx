...
export const UploadField = forwardRef<HTMLInputElement, UploadFieldProps>(({
  ...
  return (
    <div className={`flex ${inline ? 'items-start gap-4' : 'flex-col gap-1.5'} ${disabled ? 'cursor-not-allowed' : ''} ${className}`}>
      {label && (
        <label
          htmlFor={inputId}
          className={`text-sm font-medium ${disabled ? 'text-slate-500' : 'text-slate-200'} ${inline ? 'pt-2.5 min-w-[120px] shrink-0' : ''}`}
        >
          {label}
        </label>
      )}
