  ...
  const content = (
    <label
      ...
      className={`flex items-center justify-between gap-3 w-full ${
        inline
          ? ''
          : `rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 p-3 transition-colors ${
              disabled ? '' : 'hover:border-slate-300 dark:hover:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800/50'
            }`
    ...
    >
      ...
      <div className="relative flex items-center shrink-0">
        ...
        <div className={`h-6 w-11 rounded-full transition-all peer-focus-visible:ring-2 peer-focus-visible:ring-offset-2 peer-focus-visible:ring-offset-white dark:peer-focus-visible:ring-offset-slate-950 ${
          disabled ? 'bg-slate-200 dark:bg-slate-900 border-slate-300 dark:border-slate-800' :
          error
            ? 'bg-red-100 dark:bg-red-950/50 peer-checked:bg-red-500 border border-red-500/20 dark:border-red-500/30 peer-focus-visible:ring-red-500/50'
            : 'bg-slate-200 dark:bg-slate-800 peer-checked:bg-cyan-500 border border-slate-300 dark:border-slate-700 peer-checked:border-cyan-400 peer-focus-visible:ring-cyan-500/50'
        }`}></div>
        <div className={`absolute left-1 top-1 h-4 w-4 rounded-full transition-transform peer-checked:translate-x-5 shadow-sm ${
          disabled ? (checked ? 'bg-slate-400' : 'bg-slate-500') : (checked ? 'bg-white' : 'bg-white dark:bg-slate-400')
